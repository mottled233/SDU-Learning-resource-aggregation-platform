...
class csdn_spider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        if count != 10:
            count += 1
            url = self.crawl_url.format(type=self.types[0], category=self.categories[0], offset=offset - count * 10)
            yield scrapy.Request(url, headers=self.header, meta={'count': count}, callback=self.parse)

    # 首先访问csdn主页，获取cookie
    def start_requests(self):
        url = self.crawl_url.format(type=self.types[0], category=self.categories[0], offset=int(time.time() * 1000000))
